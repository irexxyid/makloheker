...
$b = trim(fgets(STDIN));
for($a=0;$a<$b;$a++){
	$email = "ganteng".rand(12345678,99999999)."@mailnesia.com";
	echo "[EMAIL : ".strtoupper($email)."]\n";
	MakeAccount:
		$asw = @file_get_contents("becom.txt");
		first();
		echo "\t**REGISTER...";
		$token = reg($email);
		//$token = login($email);
		...
		@file_put_contents("becom.txt", "$email|asdqwe123|$token\n", FILE_APPEND);
...
function currency($token){
	global $asw;
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, 'https://mobile-apps.booking.com/json/mobile.products?currency_code=USD&user_latitude=&device_id=44444444444444444444444444444444&user_longitude=&affiliate_id=332731&app_supports_gem_rewards=1&user_version=24.1-iphone&languagecode=en-us&network_type=wifi&user_os=14.0&auth_token='.$token);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
	$headers = array();
